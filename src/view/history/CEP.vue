...
// import { CEPList } from "@/config/config";
...
const getStationList = (data, type) => {
  getStation({
    ...data,
    line: formInline.value.line ? formInline.value.line : "KCE",
  }).then((res) => {
...
const setPicVideo = (params) => {
  ...
  delPic({ id: params.id, type: "logic" }).then((res) => {
    ...
    let msg = `${user.userName} 在 CEP工厂 删除了订单号为： ${params.sn} 的记录图片`;
